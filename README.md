...
- **최신 CSS로 마감** — `text-wrap: pretty`·`balance` 등 2026 Baseline 기능을 얹어 **모든 화면 폭에서** 브라우저가 줄내림·고아를 다듬게 한다(§13).
- **런타임 자동보정** — 페이지에 스크립트를 심으면 미지원 브라우저·CJK 잔여분까지 모든 폭에서 유지된다.
...
기존 페이지를 개선**할 때도, 원본의 태그·폰트 같은 조판 의도를 읽어 역할 분류에 반영한다. 나아가
`text-wrap: pretty`·`balance` 같은 **2026 Baseline CSS 기능**을 얹어, 한 폭에서만 맞던 교정을 모든 화면
폭에서 브라우저가 스스로 다듬게 위임한다(§13).
...
캔버스에 글을 붙여넣고 **다듬기 시작**을 누르면 8단계(구조 분석 → 위계 → 맞춤법·오타 → 정리 →
AI 흔적 → 가독성 → 레이아웃 → 마무리 점검)로 다듬는 과정과 각 단계의 theory §근거가 표시된다.
(AI 흔적 단계는 입력에 해당 패턴이 있을 때만, 가독성 단계 끝에는 `text-wrap: pretty`·`balance`로
모든 폭에서 유지되게 하는 §13 마감이 포함된다.)
...
- **결과 복사**: 다듬기가 끝나면 완성본을 **마크다운으로 복사**할 수 있다.
