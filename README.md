...
빈 캔버스에 글을 붙여넣고(줄바꿈으로 제목·소제목·본문·목록·인용·캡션 구분) **교정 시작**을
누르면 7단계(구조 분석 → 위계 → 맞춤법·오타 → 정리 → 가독성 → 레이아웃 → 마무리 점검)로
다듬는 과정과 각 단계의 theory §근거가 표시된다. 문서의 한글 비율을 감지해 영문 글에는 라틴
기준(§3·§4)으로 자동 분기한다. 스튜디오는 **시각적 탐색용**이며, 실제 파일 변형은 항상
`apply.mjs` 파이프라인(propose → DIFF → approve)을 거친다.
