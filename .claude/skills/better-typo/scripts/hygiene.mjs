...
const RULES = [
  ...
  },
  {
    ...
    // 경계 필수 — 따옴표 위치만으로 좌→우 짝지으면 홀수 개일 때 엉뚱한 짝을 묶는다
    // (`곧은따옴표("")는 … 예를 들어 "U.S."` → `")는 … 예를 들어 "` 를 인용구로 오인).
    // 여는 따옴표 앞은 줄시작/공백/여는괄호, 닫는 따옴표 뒤는 줄끝/공백/문장부호/조사(한글)여야 한다.
    re: /(?<=^|[\s(\[{—–-])"([^"\n]{1,100})"(?=$|[\s.,!?;:)\]}가-힣])/gmu,
    fix: (m) => '“' + m.slice(1, -1) + '”',
    message: '곧은 따옴표 → 둥근 따옴표 “ ” (theory §7)',
    // 인용구는 한 문장 안에서 끝난다 — 매치 안에 종결부호+공백이 있으면 문장을 넘어 묶은 것.
    // (`"U.S."` 처럼 약어 내부 마침표는 뒤에 공백이 없으므로 통과.)
    guard: (text, at, end) => !/[.!?]\s/u.test(text.slice(at + 1, end - 1)),
  },
  {
