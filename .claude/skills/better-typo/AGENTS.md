...
- **판단(에이전트)**: theory 참조로 아래를 만든다.
  ...
  - `css-text-modern`(§13): 최신 CSS Text 기능을 `css-rule`로 제안. §13-1 채택 — 본문 `text-wrap: pretty`(성능 비용, 긴 본문만), 제목 `text-wrap: balance`(Chromium 6·Firefox 10줄 제한), 혼합 문서 `text-autospace: normal`(Baseline 2025-11; 명세상 `insert` 동작이라 §1 nbsp 보존, `replace` 금지. 현재 구현은 `normal`뿐 — 세부 값 미지원). §13-2 진보적 향상 — `text-spacing-trim`(값은 `normal`; `trim-start`는 반대 동작, `halt`/`chws` 없는 폰트에선 비활성)·`text-box-trim`+`text-box-edge`·`hanging-punctuation`(한국어는 `allow-end`, Safari 한정)은 **`@supports`로 감싸** 제안하고 미지원 폴백을 근거에 명기. 게이트는 실제 쓸 문법과 같아야 한다(`text-box` 단축을 쓰면 `@supports (text-box: trim-both cap alphabetic)`). `text-box-trim`은 `text-box-edge` 없이는 무효 — 항상 함께. 상속 안 되므로 `:root` 말고 대상 요소에 직접(`text-spacing-trim`과 다름). 자동 삽입 금지.
