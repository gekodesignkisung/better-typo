...
- **판단(LLM = 너)** — 문맥·미학이 필요한 것:
  ...
  - `css-text-modern`(theory §13): 최신 CSS Text 기능을 `css-rule`로 제안. **§13-1(채택)** — 본문에 `text-wrap: pretty`, 제목(h1~h3)에 `text-wrap: balance` 누락 시 제안(Baseline Newly Available, 폴백 안전). **§13-2(진보적 향상)** — `text-spacing-trim`(§7 CJK 구두점)·`text-box-trim`+`text-box-edge`(§10·§6 세로 리듬)·`text-autospace`(§1 혼합 문서, glue와 조율)·`hanging-punctuation`(Safari 한정)은 **반드시 `@supports (속성: 값)`로 감싸** 제안하고 미지원 폴백(현행 유지)을 근거에 명기. **게이트는 실제 쓸 문법과 같아야 한다** — `text-box` 단축을 쓰면 `@supports (text-box: trim-both cap alphabetic)`로 물어야 longhand만 아는 엔진에서 무시되지 않는다. `text-box-trim`은 `text-box-edge` 없이는 지원 브라우저에서도 무효이므로 항상 함께 제안하고, **상속되지 않으므로 `:root`가 아니라 대상 요소에 직접** 건다(상속되는 `text-spacing-trim`과 다름). 전부 자동 삽입 금지 — 사람이 위치 승인.
